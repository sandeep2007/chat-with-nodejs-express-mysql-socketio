...
obj.userList = (data, callback) => {
    conn.query("select IF((select t3.id from " + fn.getTableName('socket_users') + " as t3 where t3.token=t1." + tokenColumn + " order by t3.date_created DESC LIMIT 1), 'ONLINE','OFFLINE') as is_online,(select date_created from " + fn.getTableName('last_seen') + " where user_id=t1.id order by date_created DESC LIMIT 1) as last_seen,(select t5.date_created from " + fn.getTableName('chat') + " as t5 where (sender_id=t1.id and t5.receiver_id='" + data.userId + "') or (sender_id='" + data.userId + "' and t5.receiver_id=t1.id) order by date_created DESC LIMIT 1) as last_chat,(select COUNT(*) from " + fn.getTableName('chat') + " where sender_id=t1.id and receiver_id='" + data.userId + "' and is_seen=0) as unread_message,t1.id,t1.email,t1.name,t1.image from " + userTableName + " as t1 where t1.id!='" + data.userId + "' and t1.email!='' order by FIELD(is_online,'ONLINE','OFFLINE'),unread_message DESC,last_chat DESC").then((userList) => {
        callback(userList);
    });
}
