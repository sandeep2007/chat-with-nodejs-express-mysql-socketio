...
obj.verifyToken = async (token, socket) => {
    ...
    let userData = await conn.query("SELECT '" + date + "' as last_seen,t1.auth_key,t1.id,t1.email from " + userTableName + " as t1 WHERE t1.auth_key='" + token + "' LIMIT 1");
...
obj.seenMessage = (data, callback) => {
    conn.query("select t1.socket_id from socket_users as t1 join " + userTableName + " as t2 on t2.auth_key=t1.token where t2.id='" + data.senderId + "' LIMIT 1").then((senderData) => {
...
obj.getUserSocketId = (userId, callback) => {
    conn.query("select t1.socket_id from socket_users as t1 join " + userTableName + " as t2 on t2.auth_key = t1.token where t2.id = '" + userId + "' LIMIT 1").then((data) => {
        if (!fn.isEmpty(data)) {
